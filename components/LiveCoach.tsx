...
const LiveCoach = () => {
  ...
  const toggleConnection = async () => {
    ...
    sessionPromiseRef.current = ai.live.connect({
        ...
        config: {
            ...
            systemInstruction: "You are a warm, empathetic coach helping the user reflect on their life purpose (Sentido) using the PERMA model. Keep responses concise and conversational.",
            speechConfig: {
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-primary-500 to-secondary-500 rounded-3xl shadow-xl text-white mb-6 flex flex-col items-center justify-center relative overflow-hidden">
        ...
        <h2 className="text-xl font-bold mb-4 z-10">Coach de Voz (En Vivo)</h2>
        <p className="text-white/80 text-center mb-6 z-10 max-w-xs">
           Habla sobre tu día y recibe orientación instantánea sobre tu propósito.
        </p>
        ...
        <button
            ...
            className={`z-10 w-20 h-20 rounded-full flex items-center justify-center transition-all transform hover:scale-105 shadow-lg border-4 ${connected ? 'bg-white text-red-500 border-red-200' : 'bg-white text-primary-600 border-primary-200'}`}
        >
        ...
        <div className="mt-4 h-6 flex items-center space-x-2 z-10">
            {connected && (
                <>
                    ...
                    <span className="text-sm font-medium ml-2">{isTalking ? 'Coach Hablando...' : 'Escuchando...'}</span>
                </>
            )}
        </div>
    </div>
